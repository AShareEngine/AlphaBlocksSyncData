...
from sync_data_system.providers.amazingdata.constants import FactorType, PeriodName, SecurityType
from sync_data_system.providers.amazingdata.provider import AmazingDataSDKConfig, AmazingDataSDKProvider
from sync_data_system.providers.amazingdata.base import BaseData, BaseDataCacheMissError
...
def main() -> int:
    ...
    if args.config and config_source != "amazingdata":
        return run_provider_config_file(
    ...
    return execute_execution_plan(plan)


def run_config_file(path: str, *, log_level_override: str | None = None) -> int:
    config_source = detect_config_source(path)
    if config_source != "amazingdata":
        return run_provider_config_file(
            source=config_source,
            config_path=path,
            log_level_override=log_level_override,
        )
    plan = load_execution_plan_from_toml(path)
    if log_level_override:
        plan = replace(plan, log_level=log_level_override)
    return execute_execution_plan(plan)


def run_provider_config_file(
    *,
    source: str,
    config_path: str,
...
def execute_execution_plan(plan: ExecutionPlan) -> int:
    logging.basicConfig(
...
def build_context(runtime_path: str | None = None, database: str = "starlight") -> SyncExecutionContext:
    del database
...
def run_registered_task(probe: Any) -> int:
    task_spec = TaskRunSpec(
        task=_provider_task_name(str(probe.name), str(probe.source)),
        codes_raw=",".join(probe.codes or probe.input_codes),
        begin_date=probe.begin_date if probe.begin_date is not None else probe.input_begin_date,
        end_date=probe.end_date if probe.end_date is not None else probe.input_end_date,
        limit=probe.limit,
        force=probe.force,
        resume=probe.resume,
    )
...
def _provider_task_name(registry_name: str, source: str) -> str:
    prefix = f"{source}."
    return registry_name[len(prefix):] if registry_name.startswith(prefix) else registry_name


def detect_config_source(path: str) -> str:
    return detect_plan_source(path, project_root=PROJECT_ROOT, default_source="amazingdata")
