...
AlphaBlocksSyncData 把数据同步拆成三层：统一同步框架、独立 provider 实现、可配置的同步计划。项目当前已经接入 `AmazingData`、`BaoStock`、`QMT`、`AKShare` 和免费美股数据源 `yfinance + FinanceDatabase`，可以通过 CLI 直接执行同步，也可以通过 API 服务触发后台任务。
...
| 模块 | 说明 |
| --- | --- |
| 数据来源 | AmazingData、BaoStock、QMT、AKShare、yfinance、FinanceDatabase |
| 数据落地 | ClickHouse |
...
python3 scripts/migrate_tushare_remove_internal_columns.py --drop-backups
```
