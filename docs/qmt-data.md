...
成功状态阻止全量重跑；不会处理其他提供商的表或状态。任务日志是追加型 `MergeTree`；checkpoint
使用 `(task_name, scope_key)` 作为业务身份。所有 QMT ClickHouse 表都不包含 `finished_at`。
...
同一规则也用于其他需要证券代码的历史、财务、合约信息、除权因子和下载任务。
实时 Tick、Level-2、完整 K 线和交易时段任务只使用当前 `沪深A股`，不会把退市代码
加入实时请求。页面批量同步未填写其他必填维度时使用以下任务默认值：

- 财务表：`Balance`、`Income`、`CashFlow`
- 指数权重：`000300.SH`
- SH/SZ 市场类任务：分别执行 `SH` 和 `SZ`
- 主力合约：`IF.CFFEX`

通过命令行、TOML 或同步接口显式传入的参数优先于这些默认值。
